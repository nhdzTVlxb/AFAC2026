...
try:
    ...
    # 在交互式环境（如 Jupyter / Kaggle Notebook）中，使用当前工作目录
...
os.environ["DATA_ROOT"] = "/kaggle/input/datasets/theunforgiven7/afac-task2/A�Ƽ�"

RUN_ROOT = os.path.dirname(os.path.abspath(__file__))
LOCAL_DATA_ROOT = r"F:\CODE\competition\AFAC_\Dataset\A推荐\A推荐"
os.environ["DATA_ROOT"] = LOCAL_DATA_ROOT
...
# ============================================================
VERSION = "T2_NN_V1_DETERMINISTIC"
# Kaggle 环境下直接用固定工作目录
SCRIPT_DIR = RUN_ROOT
...
DATA_DIR_CANDIDATES = (
    LOCAL_DATA_ROOT,
    "/kaggle/input/datasets/theunforgiven7/afac-task2/A�Ƽ�", 
    os.path.join(SCRIPT_DIR, "data"),
    os.path.abspath(os.path.join(SCRIPT_DIR, "..", "c55_lgb", "data")),
)
OUTPUT_DIR = os.path.join(RUN_ROOT, "output")
OOF_WORK_DIR = os.path.join(RUN_ROOT, "work", "oof_folds")
...
# Neural ranker and BPR training.
NN_EPOCHS = 12
NN_BATCH_SIZE = 4096
NN_LEARNING_RATE = 1e-3
NN_WEIGHT_DECAY = 1e-5
NN_GRAD_CLIP_NORM = 5.0
BPR_PAIR_SEED = 42
BPR_MAX_PAIRS_PER_GROUP = 20
...
# ===========================
# NN Ranker + True Pairwise BPR Loss
# Replace pointwise BCE ranking
# ===========================
import torch
...
def configure_determinism() -> None:
    """Fix Python, NumPy, PyTorch and CUDA sources of run-to-run variance."""
    ...
    torch.set_num_threads(NN_NUM_THREADS)
    torch.set_num_interop_threads(NN_NUM_INTEROP_THREADS)
    if DETERMINISTIC_TORCH:
        torch.use_deterministic_algorithms(True)


class NNRankerModel(nn.Module):
    def __init__(self, in_dim):
        super().__init__()
        h1, h2, h3 = NN_HIDDEN_DIMS
        d1, d2 = NN_DROPOUTS
...
def build_bpr_pairs(X, y, qid, max_pairs_per_group=BPR_MAX_PAIRS_PER_GROUP):
    """
    Construct real pairwise ranking samples:
        positive item > negative item

    Pair construction is performed inside each qid group
    to avoid leaking information between users.
    """
    ...
    rng = np.random.default_rng(BPR_PAIR_SEED)
    ...
    for _, g in order.groupby("qid"):
        ...
        pos_features.append(X[p_idx])
        neg_features.append(X[n_idx])

    if len(pos_features) == 0:
        raise ValueError("No valid BPR pairs generated")

    return (
        np.concatenate(pos_features),
        np.concatenate(neg_features)
    )
...
class NNRankerWrapper:
    ...
    def fit(
        ...
        print(
            f"BPR training pairs: {len(X_pos):,}"
        )
        ...
        dl = DataLoader(
            ...
            shuffle=True,
            num_workers=0,
            generator=torch.Generator().manual_seed(TORCH_SEED + 1),
        )

        self.model = NNRankerModel(
            X.shape[1]
        ).to(self.device)
        ...
        for ep in range(epochs):
            ...
            for pos_x, neg_x in dl:

                pos_x = pos_x.to(self.device)
                neg_x = neg_x.to(self.device)

                pos_score = self.model(pos_x)
                neg_score = self.model(neg_x)
...
def train_rankers(oof):
    ...
    """
    Neural Ranking version with true Pairwise BPR Loss.
    """
    ...
    qidt = mt["qid"].to_numpy()


    print("\nTraining Neural Ranker with BPR loss...")
    ...
    ms["lgb_score"] = nn_ranker.predict(Xv)

    n, h = ndcg_hit_from_frame(
        ms,
        "lgb_score"
    )

    print(
        f"\nMeta-validation NN+BPR: "
        f"Hit@10={h:.4f} "
        f"NDCG@10={n:.4f}"
    )
...
def main():
    ...
    config_snapshot = {
        ...
        "runtime": {
            ...
            "n_folds": N_FOLDS,
            "random_state": RANDOM_STATE,
            "pythonhashseed_value": REQUIRED_PYTHONHASHSEED,
            "torch_seed": TORCH_SEED,
            "deterministic_torch": DETERMINISTIC_TORCH,
            "cublas_workspace_config": CUBLAS_WORKSPACE_CONFIG,
            "nn_device": NN_DEVICE,
            "nn_num_threads": NN_NUM_THREADS,
            "nn_num_interop_threads": NN_NUM_INTEROP_THREADS,
        ...
        "neural_ranker": {
            ...
            "weight_decay": NN_WEIGHT_DECAY,
            "grad_clip_norm": NN_GRAD_CLIP_NORM,
            "bpr_pair_seed": BPR_PAIR_SEED,
            "bpr_max_pairs_per_group": BPR_MAX_PAIRS_PER_GROUP,
            "hidden_dims": list(NN_HIDDEN_DIMS),
            "dropouts": list(NN_DROPOUTS),
        },
        "candidate": {
    ...
    if args.print_config:
        print(json.dumps(config_snapshot, ensure_ascii=True, indent=2))
        return

    print(f"=== {VERSION} ===")
    print(
        f"PYTHONHASHSEED={os.environ.get('PYTHONHASHSEED')}, "
        f"torch_seed={TORCH_SEED}, deterministic={DETERMINISTIC_TORCH}, "
        f"device={NNRankerWrapper().device}, folds={N_FOLDS}, "
