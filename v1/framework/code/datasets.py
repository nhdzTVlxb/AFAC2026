...
class NegativeSamplingDataset(Dataset):
    ...
    def __init__(self, item_seqs, targets, seq_lengths, num_items,
        ...
        self.histories = []
        for seq, target in zip(self.item_seqs.numpy(), self.targets.numpy()):
            used = set(int(x) for x in seq if x > 0)
            used.add(int(target))
            self.histories.append(used)
        ...
        probs = self.item_freq[1:]
        if probs.sum() <= 0:
            probs = np.ones(num_items, dtype=np.float32)
        self.popularity_probs = probs / probs.sum()

    def __len__(self):
        return len(self.item_seqs)

    def _sample_negative(self, forbidden_items):
        """采样负样本"""
        negatives = []
        max_retry = max(50, self.neg_samples * 20)
        while len(negatives) < self.neg_samples:
            for _ in range(max_retry):
                if self.strategy == 'popularity':
                    neg = int(np.random.choice(self.num_items, p=self.popularity_probs) + 1)
                else:
                    neg = int(np.random.randint(1, self.num_items + 1))
                if neg not in forbidden_items and neg not in negatives:
                    negatives.append(neg)
                    break
            else:
                candidates = [i for i in range(1, self.num_items + 1)
                              if i not in forbidden_items and i not in negatives]
                if not candidates:
                    negatives.append(int(np.random.randint(1, self.num_items + 1)))
                else:
                    negatives.append(int(np.random.choice(candidates)))
        return np.asarray(negatives, dtype=np.int64)

    def __getitem__(self, idx):
        neg_targets = self._sample_negative(self.histories[idx])
