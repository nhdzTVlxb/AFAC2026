...
def preprocess_features(features, method='row'):
    """预处理节点特征矩阵。

    method:
        none: 不处理
        row: 行归一化，适合稀疏计数/多热特征
        standard: 按列标准化
        binary: 非零特征置为1
    """
    ...
    if method == 'binary':
        if sp.issparse(features):
            ...
            features.data = np.ones_like(features.data, dtype=np.float32)
            return features
        return (np.asarray(features) > 0).astype(np.float32)

    if method == 'row':
        if sp.issparse(features):
            features = features.tocsr(copy=True).astype(np.float32)
    ...
    raise ValueError(f"不支持的特征预处理方法: {method}")


def drop_edges(adj, drop_rate=0.0, seed=42, keep_self_loop=False):
    """随机丢弃部分边，用作图数据增强。"""
    if drop_rate <= 0:
        return adj
    ...
    keep_mask = rng.random(coo.nnz) >= drop_rate
    if keep_self_loop:
        keep_mask = keep_mask | (coo.row == coo.col)
...
def apply_feature_mask(features, mask_rate=0.0, seed=42):
    """随机屏蔽非零特征，用作特征层面的数据增强。"""
    if mask_rate <= 0:
        return features
    rng = np.random.default_rng(seed)
    if sp.issparse(features):
        features = features.tocsr(copy=True)
        keep_mask = rng.random(features.data.shape[0]) >= mask_rate
        features.data = features.data * keep_mask.astype(features.data.dtype)
        features.eliminate_zeros()
        return features
    features = np.asarray(features, dtype=np.float32).copy()
    mask = rng.random(features.shape) < mask_rate
    features[mask] = 0.0
    return features
