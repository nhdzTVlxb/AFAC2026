...
from utils import (
    ...
    preprocess_features, drop_edges, apply_feature_mask,
...
def encode_sequence(model, model_type, item_seqs, seq_lens=None):
    """兼容GRU4Rec和SASRec的序列编码调用。"""
    if model_type == 'gru4rec':
        return model(item_seqs, seq_lens)
    return model(item_seqs)
...
def run_with_ema_weights(model, ema_state, fn):
    """临时切换到EMA权重执行fn，然后恢复训练中权重。"""
    if ema_state is None:
        return fn()
    current_state = {
        name: value.detach().clone()
...
def parse_args():
    ...
    parser.add_argument('--normalize', type=str, default='symmetric',
                        ...
                        help='邻接矩阵归一化方式')
    parser.add_argument('--feature_norm', type=str, default='row',
                        choices=['none', 'row', 'standard', 'binary'],
                        help='节点特征预处理方式')
    parser.add_argument('--dropedge', type=float, default=0.0,
                        help='训练时随机丢弃边的比例(Task 1)')
    parser.add_argument('--feature_mask_rate', type=float, default=0.0,
                        help='训练时随机屏蔽非零特征的比例(Task 1)')
    ...
    parser.add_argument('--neg_samples', type=int, default=1,
                        help='负采样数量(推荐任务)')
    parser.add_argument('--neg_sampling_strategy', type=str, default='popularity',
...
def train_task1(args):
    ...
    # 特征预处理与增强
    features_np = preprocess_features(data['features'], method=args.feature_norm)
    features_np = apply_feature_mask(features_np, mask_rate=args.feature_mask_rate, seed=args.seed)
    if args.dropedge > 0:
        logging.info(f"启用DropEdge: drop_rate={args.dropedge}")
    adj_np = drop_edges(data['adj'], drop_rate=args.dropedge, seed=args.seed)

    # 特征转tensor
    if sp.issparse(features_np):
        features = sparse_to_torch(features_np, device=device)
    else:
        features = torch.FloatTensor(features_np).to(device)

    # 邻接矩阵归一化
    if args.normalize == 'symmetric':
        adj = normalize_adj(adj_np).to(device)
    elif args.normalize == 'random_walk':
        adj = random_walk_normalize(adj_np).to(device)
    else:
        if sp.issparse(adj_np):
            adj = sparse_to_torch(adj_np, device=device)
        else:
    ...
    use_ema = args.ema_decay is not None and 0.0 < args.ema_decay < 1.0
    ema_state = init_ema_state(model) if use_ema else None
    if use_ema:
        logging.info(f"启用EMA: decay={args.ema_decay}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            def evaluate_task1():
                eval_logits = model(features, adj)
                eval_loss = criterion(eval_logits[val_idx_t], labels[val_idx_t]).item()
                eval_acc = compute_accuracy(eval_logits[val_idx_t], labels[val_idx_t])
                return eval_logits, eval_loss, eval_acc

            _, val_loss, val_acc = run_with_ema_weights(model, ema_state, evaluate_task1)
        ...
        if val_acc > best_val_acc:
            ...
            save_dict = {
                'epoch': epoch,
                'model_state_dict': ema_state if use_ema else model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'val_acc': val_acc,
                'ema_decay': args.ema_decay if use_ema else 0.0,
                'args': vars(args)
            }
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            if args.loss_type == 'bpr':
                ...
                # 获取序列表示
                seq_repr = encode_sequence(model, args.model_type, item_seqs, seq_lens)
                ...
                # 负样本分数
                neg_emb = model.item_embedding(neg_items)  # (batch, neg_samples, embed_dim)
                neg_score = (seq_repr.unsqueeze(1) * neg_emb).sum(dim=-1)  # (batch, neg_samples)
        ...
        with torch.no_grad():
            def evaluate_task2():
                ...
                for batch in val_loader:
                    ...
                    top_indices = (top_indices + 1).cpu().numpy()

                    predictions.extend(top_indices.tolist())
                    targets_list.extend(targets.cpu().numpy().tolist())
                return predictions, targets_list

            val_predictions, val_targets_list = run_with_ema_weights(
                model, ema_state, evaluate_task2
            )
