...
输出格式:
    - A1.csv: test_idx,label
    - A2.csv: uid,prediction
...
from utils import (
    ...
    preprocess_features, set_seed, get_device, setup_logger
)
...
def prepare_graph_tensors(data, checkpoint_path, device):
    """按训练时保存的参数准备图特征和邻接矩阵。"""
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
    saved_args = checkpoint.get('args', {})
    feature_norm = saved_args.get('feature_norm', 'row')
    normalize_type = saved_args.get('normalize', 'symmetric')

    features_np = preprocess_features(data['features'], method=feature_norm)
...
def infer_task1(args):
    ...
    # 2. 数据预处理
    features, adj = prepare_graph_tensors(data, args.checkpoint, device)
    test_idx_t = torch.LongTensor(test_idx).to(device)

    # 3. 加载模型。可选多个checkpoint做logits平均集成。
    checkpoint_paths = args.ensemble_checkpoints or [args.checkpoint]
    ...
    with torch.no_grad():
        ...
        logits = logits_sum / len(checkpoint_paths)
...
def infer_task2_v2(args):
    ...
    # 2. 加载模型。可选多个checkpoint做分数平均集成。
    checkpoint_paths = args.ensemble_checkpoints or [args.checkpoint]
    models = []
    model_args_list = []
    for ckpt_path in checkpoint_paths:
        model, model_args = load_model_from_checkpoint(ckpt_path, device)
        models.append(model)
        model_args_list.append(model_args)
    model_type = model_args_list[0].get('model_type', 'gru4rec')
    max_len = model_args_list[0].get('max_len', 50)
    ...
    with torch.no_grad():
        for user_id, group in test_df.groupby(user_col):
            ...
            # 计算所有物品分数，多个模型取平均
            scores = None
            for model, model_args in zip(models, model_args_list):
                current_type = model_args.get('model_type', model_type)
                if current_type == 'gru4rec':
                    seq_repr = model(item_seq, seq_len_t)
                else:
                    seq_repr = model(item_seq)
                all_item_emb = model.item_embedding.weight[1:]
                cur_scores = seq_repr @ all_item_emb.T
                scores = cur_scores if scores is None else scores + cur_scores
            ...
            # 取Top-K
            _, top_indices = torch.topk(scores[0], k=min(args.topk, scores.size(1)))
