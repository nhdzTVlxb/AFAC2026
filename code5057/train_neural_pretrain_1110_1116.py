...
def main():
    ...
    configs = [
        {"name": "output1110_sasrec_pre_testmix", "kind": "sasrec", "modes": ["testmix", "last3", "empty"], "seed": 10, "epochs": 18, "dim": 96, "dropout": 0.22, "lr": 5e-4},
        {"name": "output1111_sasrec_pre_shortheavy", "kind": "sasrec", "modes": ["testmix", "last3", "last1", "empty"], "seed": 11, "epochs": 18, "dim": 96, "dropout": 0.24, "lr": 5e-4},
        {"name": "output1112_gru_pre_testmix", "kind": "gru", "modes": ["testmix", "last3", "empty"], "seed": 12, "epochs": 16, "dim": 96, "dropout": 0.18, "lr": 5e-4},
        {"name": "output1113_sasrec_pre_deep", "kind": "sasrec", "modes": ["testmix", "last3", "empty"], "seed": 13, "epochs": 18, "dim": 96, "layers": 3, "dropout": 0.26, "lr": 4e-4},
    ]
    preds = []
    for cfg in configs:
        preds.append(train_one_pretrained(cfg, tr_rows, va_rows, maps, sample, test, args.output_dir, device, emb))
    fused = rank_fuse(preds, [1.15, 1.0, 0.85, 1.0])
    save_submission(sample, fused, os.path.join(args.output_dir, "output1114_pre_nn_fusion", "A2.csv"))
    ...
    try:
        ...
        fused2 = rank_fuse(pred_maps, [1.25, 1.0, 0.85, 0.75, 0.9])
        save_submission(sample, fused2, os.path.join(args.output_dir, "output1115_pre_x_1100_fusion", "A2.csv"))
