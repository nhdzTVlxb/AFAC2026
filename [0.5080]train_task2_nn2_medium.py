...
"""V44: PMI特征 + 中型双残差网络 + NDCG 早停排序器。

优化点：
1. RecallStats: 新增 PMI (Pointwise Mutual Information) 召回与特征计算，替代Jaccard的劣势。
2. Medium NNRanker: 两个正则化瓶颈残差块与低维乘性交互分支。
3. Stability: 对 Lift/Log 等特征进行 Clipping，防止极值破坏训练。
4. Sampling: (隐式) 通过新特征强化模型对热门物品的感知。
"""
...
# ============================================================
VERSION = "T2_NN_V44_MEDIUM_RESIDUAL_NDCG_ES"
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
...
MODEL_PATH = os.path.join(OUTPUT_DIR, "model_nn_medium.pt")
...
SUBMISSION_FILENAME = "A2_medium.csv"
...
NN_LEARNING_RATE = float(os.environ.get("NN_LEARNING_RATE", "0.0002"))
...
class RecallStats:
    ...
    # V41: PMI Implementation
    def _build_pmi_cache(self):
        cache = {}
        N = float(self.n_rows) # Total number of users/sessions
        for a, coocs in self.item_cooc.items():
            ...
            for b, cooc in coocs.items():
                ...
                if occ_a > 0 and occ_b > 0 and cooc > 0:
                    ...
                    #     = log( (C(a,b)/N) / ((C(a)/N) * (C(b)/N)) )
                    #     = log( C(a,b) * N / (C(a) * C(b)) )
                    val = math.log((cooc * N) / (occ_a * occ_b))
                    cache[(a, b)] = val
                else:
                    cache[(a, b)] = -10.0 # Low score for no co-occurrence
        self._pmi_cache = cache

    def _i2i_pmi(self, item_a, item_b):
        if item_a == item_b: return 0.0 # Self doesn't give extra info
        a, b = (item_a, item_b) if item_a < item_b else (item_b, item_a)
    ...
    def pair_features(self, raw, dedup, cat_vals, iid, meta):
        ...
        if dedup:
            for item in dedup:
                ...
                if pmi > -5.0: # Filter noise
                    i2i_pmis.append(pmi)
            f["i2i_pmi_max"] = max(i2i_pmis) if i2i_pmis else -10.0
            f["i2i_pmi_sum"] = float(np.sum(i2i_pmis)) if i2i_pmis else 0.0
            f["i2i_pmi_last1"] = self._i2i_pmi(dedup[-1], iid) if dedup else 0.0
            
            # Keep Jaccard as baseline
            i2i_sims = []
            for item in dedup:
            ...
            f["i2i_jaccard_last1"] = self._i2i_jaccard(dedup[-1], iid) if dedup else 0.0
             ...
             f["i2i_pmi_max"] = -10.0
             f["i2i_pmi_sum"] = 0.0
             f["i2i_pmi_last1"] = -10.0
             f["i2i_jaccard_max"] = 0.0
             f["i2i_jaccard_sum"] = 0.0
             f["i2i_jaccard_last1"] = 0.0
...
# ============================================================
# V44: Medium two-block residual ranker
# ============================================================
class BottleneckResidualBlock(nn.Module):
    def __init__(self, dim=320, bottleneck=160, dropout=0.10):
        super().__init__()
...
class MediumResidualRanker(nn.Module):
    def __init__(self, in_dim, feature_mean=None, feature_std=None, width=384, interaction_dim=64):
        super().__init__()
        ...
        self.residuals = nn.Sequential(
            BottleneckResidualBlock(width, bottleneck=192, dropout=0.12),
            BottleneckResidualBlock(width, bottleneck=128, dropout=0.10),
        )
    ...
    def forward(self, x):
        ...
        return main_score + 0.25 * interaction_score + 0.08 * wide_score
...
class NNRankerWrapper:
    ...
    def fit(
        ...
        self.model = MediumResidualRanker(
            self.in_dim,
    ...
    def load(cls, path):
        ...
        wrapper.model = MediumResidualRanker(wrapper.in_dim).to(wrapper.device)
...
def train_rankers(oof):
    ...
    print("\nTraining medium residual ranker with direct NDCG early stopping...")
    ...
    print(f"\nMeta-validation Medium NN+BPR: Hit@10={h:.4f} NDCG@10={n:.4f}")
